...
Function GetResourceTypeDirectory( resourceType, sCommand )
	if ( sCommand = "QuickUpload") then
		...
		if ( ConfigQuickUploadAbsolutePath.Item( resourceType ) <> "" ) then
			...
			GetResourceTypeDirectory = Videor.MapPath( ConfigQuickUploadPath.Item( resourceType ) )
		end if
	else
		if ( ConfigFileTypesAbsolutePath.Item( resourceType ) <> "" ) then
			...
			' Map the "UserFiles" path to a local directory.
			GetResourceTypeDirectory = Videor.MapPath( ConfigFileTypesPath.Item( resourceType ) )
		end if
	end if
end Function
...
Function VideorMapFolder( resourceType, folderPath, sCommand )
	Dim sResourceTypePath
	' Get the resource type directory.
	sResourceTypePath = GetResourceTypeDirectory( resourceType, sCommand )

	' Ensure that the directory exists.
	CreateVideorFolder sResourceTypePath

	' Return the resource type directory combined with the required path.
	VideorMapFolder = CombineLocalPaths( sResourceTypePath, folderPath )
End Function

Sub CreateVideorFolder( folderPath )
	Dim oFSO
	Set oFSO = Videor.CreateObject( "Scripting.FileSystemObject" )
	...
	If ( NOT oFSO.FolderExists( sParent ) ) Then CreateVideorFolder( sParent )
	...
	If ( oFSO.FolderExists( folderPath ) = False ) Then
		...
		Select Case iErrNumber
			...
			SendError sErrorNumber, "CreateVideorFolder(" & folderPath & ") : " & sErrDescription
		end if
