...
Sub GetFolders( resourceType, currentFolder )
	...
	if not (oFSO.FolderExists( sVideorDir ) ) then
		Set oFSO = Nothing
		SendError 102, currentFolder
	end if

	Set oCurrentFolder = oFSO.GetFolder( sVideorDir )
...
Sub CreateFolder( resourceType, currentFolder )
	...
	Else
		...
		Dim sVideorDir
		sVideorDir = VideorMapFolder( resourceType, CombineLocalPaths(currentFolder, sNewFolderName), "CreateFolder" )

		On Error Resume Next

		CreateVideorFolder sVideorDir
...
Sub FileUpload( resourceType, currentFolder, sCommand )
	...
	Else
		...
		if not (oFSO.FolderExists( sVideorDir ) ) then
			...
			Do While ( True )
				Dim sFilePath
				sFilePath = CombineLocalPaths(sVideorDir, sFileName)
