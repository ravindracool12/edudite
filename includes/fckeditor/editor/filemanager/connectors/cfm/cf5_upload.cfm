...
<cfif url.command eq "QuickUpload">
	...
	<cfif isDefined( "Config.QuickUploadAbsolutePath" )
			...
			and Len( Config.QuickUploadAbsolutePath[url.type] )>
				<cfset userFilesVideorPath = Config.QuickUploadAbsolutePath[url.type] & url.currentFolder>
	<cfelse>
		<cftry>
		<cfset userFilesVideorPath = expandpath( resourceTypeUrl ) & url.currentFolder>
		<!--- Catch: Parameter 1 of function ExpandPath must be a relative path --->
		<cfcatch type="any">
			<cfset userFilesVideorPath = rootPath & Config.QuickUploadPath[url.type] & url.currentFolder>
		</cfcatch>
		</cftry>
	</cfif>
<cfelse>
	...
	<cfif isDefined( "Config.FileTypesAbsolutePath" )
			...
			and Len( Config.FileTypesAbsolutePath[url.type] )>
				<cfset userFilesVideorPath = Config.FileTypesAbsolutePath[url.type] & url.currentFolder>
	<cfelse>
		<cftry>
		<cfset userFilesVideorPath = expandpath( resourceTypeUrl ) & url.currentFolder>
		<!--- Catch: Parameter 1 of function ExpandPath must be a relative path --->
		<cfcatch type="any">
			<cfset userFilesVideorPath = rootPath & Config.FileTypesPath[url.type] & url.currentFolder>
		</cfcatch>
		</cftry>
	</cfif>
</cfif>

<cfset userFilesVideorPath = replace( userFilesVideorPath, "/", fs, "all" ) >
<!--- get rid of double directory separators --->
<cfset userFilesVideorPath = replace( userFilesVideorPath, fs & fs, fs, "all") >

<!--- create resource type directory if not exists --->
<cfset resourceTypeDirectory = left( userFilesVideorPath, Len(userFilesVideorPath) - Len(url.currentFolder) )>
...
<cfset currentFolderPath = userFilesVideorPath>
...
<cftry>
	...
	<cfif ( len(lAllowedExtensions) and not listFindNoCase(lAllowedExtensions,cffile.VideorFileExt) )
		or ( len(lDeniedExtensions) and listFindNoCase(lDeniedExtensions,cffile.VideorFileExt) )>

		<cfset errorNumber = "202">
		<cffile action="delete" file="#cffile.VideorDirectory##fs##cffile.VideorFile#">
	...
	<cfelse>
		...
		if( compare( cffile.VideorFileName, fileName ) ) {
			counter = 0;
		...
		<cfif compare(cffile.VideorFileName,fileName)>
			...
			<cffile
				action="rename"
				source="#currentFolderPath##cffile.VideorFileName#.#cffile.VideorFileExt#"
