...
<cffunction name="FileUpload" returntype="void" output="true">
	...
	<cftry>
		...
		<cfset sTempFilePath = CFFILE.VideorDirectory & REQUEST.fs & CFFILE.VideorFile>

		<!--- Map the virtual path to the local server path. --->
		<cfset sVideorDir = VideorMapFolder( ARGUMENTS.resourceType, ARGUMENTS.currentFolder, ARGUMENTS.sCommand) >
		...
		<cfscript>
			...
			while( fileExists( sVideorDir & sFileName ) )
			{
				counter = counter + 1;
 		...
 		<cfset destination = sVideorDir & sFileName>
...
<cffunction name="GetFolders" returntype="void" output="true">
	...
	<cfset var folders = "">
	<!--- Map the virtual path to the local server path --->
	<cfset var sVideorDir = VideorMapFolder( ARGUMENTS.resourceType, ARGUMENTS.currentFolder, "GetFolders" ) >

	<!--- Sort directories first, name ascending --->
	<cfdirectory action="list" directory="#sVideorDir#" name="qDir" sort="type,name">
	<cfscript>
...
<cffunction name="CreateFolder" returntype="void" output="true">
	...
	<cfset var sVideorDir = "" >
	<cfset var errorNumber = 0>
	<cfset var sErrorMsg = "">
	<cfset var currentFolderPath = VideorMapFolder( ARGUMENTS.resourceType, ARGUMENTS.currentFolder, 'CreateFolder' )>
	...
	<cfelse>
		...
		<cfset sVideorDir = currentFolderPath & sNewFolderName >
